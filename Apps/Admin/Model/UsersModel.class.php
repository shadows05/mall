    ...
    /**
	 ...
	 public function insert(){
		...
		if($this->checkEmpty($data)){
			...
			$data["userIdcard"] = I("userIdcard");
			$data["userWebchat"] = I("userWebchat");
	 ...
	 public function edit(){
		 ...
		 if(I("userIdcard")!=''){
			 $hasUserEmail = self::checkLoginKey(I("userIdcard"),$id);
			 if($hasUserEmail['status']<=0){
				 $rd = array('status'=>-2);
				 return $rd;
			 }
		 ...
		 if(I("userWebchat")!=''){
			 $hasUserEmail = self::checkLoginKey(I("userWebchat"),$id);
			 if($hasUserEmail['status']<=0){
				 $rd = array('status'=>-2);
				 return $rd;
			 }
...
//		}
	 ...
	 public function checkLoginKey($val,$id = 0){
	 	$rd = array('status'=>-1);
	 	if($val=='')return $rd;
	 	$sql = " (loginName ='%s' or userPhone ='%s' or userIdcard='%s' or userWebchat='%s' or userEmail='%s') and userFlag=1";
	 	$keyArr = array($val,$val,$val,$val,$val);
	 	if($id>0){
	 		$sql.=" and userId!=".$id;
	 	}
	 	$m = M('users');
